...
const highLatitudeAqrabulAyyamResolver = (date, coordinates, fajrAngle) => {
...
class CalculationParameters {
  ...
  highLatitudeFajrRule = (() => HighLatitudeFajrRule.Default)();
  ...
  nightPortions() {
    ...
    switch (this.highLatitudeRule) {
      ...
      default:
        throw new Error(`Invalid high latitude rule found when attempting to compute night portions: ${this.highLatitudeRule}`);
    }
  }
}
...
class PrayerTimes {
  ...
  constructor(coordinates, date, calculationParameters) {
    ...
    asrTime = new TimeComponents(solarTime.afternoon(shadowLength(calculationParameters.madhab))).utcDate(date);
    const tomorrowSunrise = new TimeComponents(tomorrowSolarTime.sunrise).utcDate(tomorrow);
    const night = (Number(tomorrowSunrise) - Number(sunsetTime)) / 1000;
    fajrTime = new TimeComponents(solarTime.hourAngle(-1 * calculationParameters.fajrAngle, false)).utcDate(date);
    ...
    const safeFajr = function () {
      if (calculationParameters.method === 'MoonsightingCommittee') {
        ...
        return fajrTime;
      } else if (calculationParameters.highLatitudeFajrRule === HighLatitudeFajrRule.AqrabYaum && isNaN(solarTime.hourAngle(-1 * calculationParameters.fajrAngle, false))) {
        const lastFajrDate = highLatitudeAqrabulAyyamResolver(date, coordinates, calculationParameters.fajrAngle);
        const lastFajrSolarTime = new SolarTime(lastFajrDate, coordinates);
        return new TimeComponents(lastFajrSolarTime.hourAngle(-1 * calculationParameters.fajrAngle, false)).utcDate(date);
      } else {
    ...
    if (calculationParameters.ishaInterval > 0) {
      ...
      ishaTime = new TimeComponents(solarTime.hourAngle(-1 * calculationParameters.ishaAngle, true)).utcDate(date);
    ...
    if (calculationParameters.maghribAngle) {
      const angleBasedMaghrib = new TimeComponents(solarTime.hourAngle(-1 * calculationParameters.maghribAngle, true)).utcDate(date);
...
export { CalculationMethod, CalculationParameters, Coordinates, HighLatitudeFajrRule, HighLatitudeRule, Madhab, PolarCircleResolution, Prayer, PrayerTimes, qibla as Qibla, Rounding, Shafaq, SunnahTimes };
